...
public function get_resena() { return $this->_resena; }

public function get_autor() { return $this->_autor;  }

public function get_fechaDesde() { return $this->_fechaDesde; }

public function get_fechaHasta() {  return $this->_fechaHasta; }

public function get_puntaje() { return $this->_puntaje; }

public function get_detallesExtra() {  return $this->_detallesExtra; }

public function get_numeroSitios() { return $this->_numeroSitios; }

public function get_precio() { return $this->_precio; }

public function get_duracion() { return $this->_duracion; }

public function get_director() { return $this->_director; }

public function get_afiche() { return $this->_afiche; }

public function set_id($_id) {  $this->_id = $_id; }

public function set_titulo($_titulo) { $this->_titulo = $_titulo; }

public function set_resena($_resena) { $this->_resena = $_resena; }

public function set_autor($_autor) { $this->_autor = $_autor; }

public function set_fechaDesde($_fechaDesde) { $this->_fechaDesde = $_fechaDesde; }

public function set_fechaHasta($_fechaHasta) { $this->_fechaHasta = $_fechaHasta; }

public function set_puntaje($_puntaje) { $this->_puntaje = $_puntaje; }

public function set_detallesExtra($_detallesExtra) {  $this->_detallesExtra = $_detallesExtra;  }

public function set_numeroSitios($_numeroSitios) {  $this->_numeroSitios = $_numeroSitios; }

public function set_precio($_precio) {  $this->_precio = $_precio; }

public function set_duracion($_duracion) {  $this->_duracion = $_duracion; }

public function set_director($_director) { $this->_director = $_director;  }

public function set_afiche($_afiche) { $this->_afiche = $_afiche;  }
...
$sql = "INSERT INTO 'Obras' ('titulo', 'resena', 'autor', 'fecha_desde', 'fecha_hasta', 'puntaje', 'detalles_extra', 'numeroSitios', " . "'precio', 'duracion', 'director', afiche') VALUES ( '" . $v_titulo . "', '" . $v_resena . "', '" . $v_autor . "', '" . $v_fechadesde . "', '" .  $v_fechahasta  . "', '" .  $v_puntaje  . "', '" .  $v_detallesExtra  . "', '" .  $v_numeroSitios  . "', '" .  $v_precio  . "', '" .  $v_duracion  . "', '" .  $v_director  . "', '" .  $v_afiche . ")";
...
public function getClaseDeObra() {
  ...
  if ($puntaje>=50)  {
  	if($puntaje>=100)   {
    	if($puntaje>=150)    {
      	if($puntaje>=200)    {  	$clase='nihyaku'; } 
       	else  {   $clase="hyakugojuu"; }
   		} else { $clase="hyaku";  }
    } else {   $clase='gojuu';  }
  } else {     $clase='rei';    }
	return $clase;
...
public function _updateObra($v_id, $v_titulo, $v_resena, $v_autor, $v_fechadesde, $v_fechahasta, $v_puntaje, $v_detallesExtra, $v_numeroSitios, $v_precio, $v_duracion, $v_director, $v_afiche) {
		$sql = "UPDATE 'Obras' SET titulo = '" . $v_titulo ."', resena = '" . $v_resena . "', autor = '" . $v_autor . "', fecha_desde = '" . $v_fechadesde . "', " . "fecha_hasta = '" . $v_fechahasta . "', puntaje = '" . $v_puntaje . "', detalles_extra = '" . $v_detallesExtra . "', numeroSitios = '" . $v_numeroSitios . "', " . "precio = '" . $v_precio . "', duracion = '" . $v_duracion . "', director = '" . $v_director . "', afiche = '" . $v_afiche . "' " . "WHERE idObras = '" . $v_id . "'";

		mysql_query($sql) OR die (mysql_error());
		 
}
	
public function _deleteObra($id){
		$sql = "DELETE FROM 'Obras' WHERE idObras = '" . $id . "'";
		mysql_query($sql) OR die (mysql_error());
}
